...
public class SendNotificationProcessingService extends
    ...
    protected List<SendNotificationErrorDTO> consumeRow(long lineNumber, SendNotificationIngestionFlowFileDTO row,
        ...
        if (createNotificationRequest.getRecipients().getFirst().getPayments() != null
            ...
            return List.of(buildErrorDto(ingestionFlowFile, lineNumber, row, FileErrorCode.NOTIFICATION_ALREADY_PROCESSED.name(),
        ...
        if (createResponse != null) {
            ...
            if (sendNotificationDTO != null) {
                ...
                Objects.requireNonNull(createNotificationRequest.getDocuments())
                        .stream()
        ...
        return List.of(buildErrorDto(ingestionFlowFile, lineNumber, row,
                FileErrorCode.NOTIFICATION_NOT_PROCESSED.name(),
                FileErrorCode.NOTIFICATION_NOT_PROCESSED.getMessage()));
    }

    @Override
    protected SendNotificationErrorDTO buildErrorDto(IngestionFlowFile ingestionFlowFile, long lineNumber, SendNotificationIngestionFlowFileDTO row,
    ...
    private boolean checkSendNotificationAlreadyExists(Long organizationId, List<Payment> payments) {
        return payments.stream()
                .map(payment -> {
                    try {
                        return sendNotificationService.findSendNotificationByOrgIdAndNav(organizationId, Objects.requireNonNull(payment.getPagoPa()).getNoticeCode());
                    } catch (Exception e) {
                        return null;
                    }
                })
