...
public class ReceiptMapper {
    ...
    public ReceiptWithAdditionalNodeDataDTO map(IngestionFlowFile ingestionFlowFile, ReceiptIngestionFlowFileDTO receipt) {
        return ReceiptWithAdditionalNodeDataDTO.builder()
                ...
                .noticeNumber(DebtPositionUtilities.iuv2nav(receipt.getCodIuv()))
                .orgFiscalCode(receipt.getIdentificativoDominio())
                .paymentReceiptId(receipt.getIdentificativoMessaggioRicevuta())
                .paymentDateTime(Utilities.toOffsetDateTime(receipt.getDataOraMessaggioRicevuta()))
                ...
                .outcome(!Objects.equals(receipt.getCodiceEsitoPagamento(), "1") ? "OK" : receipt.getCodiceEsitoPagamento())
                .paymentAmountCents(Utilities.bigDecimalEuroToLongCentsAmount(receipt.getImportoTotalePagato()))
                .creditorReferenceId(receipt.getIdentificativoUnivocoVersamento())
                .description(receipt.getCausaleVersamento())
                .paymentNote("9/".concat(receipt.getDatiSpecificiRiscossione()))
    ...
    private ReceiptTransferDTO buildTransferDTO(ReceiptIngestionFlowFileDTO receipt) {
        return ReceiptTransferDTO.builder()
                .transferAmountCents(Utilities.bigDecimalEuroToLongCentsAmount(receipt.getImportoTotalePagato()))
                .remittanceInformation(receipt.getCausaleVersamento())
                .idTransfer(receipt.getIndiceDatiSingoloPagamento())
                .fiscalCodePA(receipt.getCodFiscalePa1())
                .companyName(receipt.getDeNomePa1())
                .transferCategory(receipt.getCodTassonomicoDovutoPa1())
                .mbdAttachment(receipt.getBlbRtDatiPagDatiSingPagAllegatoRicevutaTest())
                .build();
    }
}