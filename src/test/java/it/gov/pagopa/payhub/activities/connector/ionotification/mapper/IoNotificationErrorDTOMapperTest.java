...
class IoNotificationErrorDTOMapperTest {

  @Test
  void whenMapThenReturnPuErrorDTO() {
    // Given
    IoNotificationErrorDTO errorDTO = TestUtils.getPodamFactory().manufacturePojo(IoNotificationErrorDTO.class);
    Objects.requireNonNull(errorDTO.getFields());

    // When
    PuErrorDTO result = IoNotificationErrorDTOMapper.map(errorDTO);
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      it.gov.pagopa.pu.ionotification.dto.generated.ErrorFieldDTO expectedErrorFieldDTO = errorDTO.getFields().get(i);
